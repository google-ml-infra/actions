...
def make_result(config_id, metrics_dict):
  """Creates a BenchmarkResult proto from a dict of {name: {stat_enum: value}}."""
...
def test_load_results_parsing(tmp_path):
  ...
  # Run the loader
  pairs = ab_analyzer_lib.load_results(str(tmp_path))
...
def test_get_comparison_config_specific():
  """Test retrieving specific thresholds from proto matrix."""
...
def test_generate_report_pass():
  ...
  # Setup results: Baseline=100, Experiment=99 (1% improvement)
  base = make_result("test", {"latency": {metric_pb2.Stat.MEAN: 100.0}})
  exp = make_result("test", {"latency": {metric_pb2.Stat.MEAN: 99.0}})

  pairs = {"test": {"baseline": base, "experiment": exp}}
  matrix = {}  # Defaults: 5% threshold, LESS

  report, success = ab_analyzer_lib.generate_report(pairs, matrix)
...
def test_generate_report_fail_latency():
  ...
  # Setup results: Baseline=100, Experiment=110 (10% regression)
  base = make_result("test", {"latency": {metric_pb2.Stat.P99: 100.0}})
  exp = make_result("test", {"latency": {metric_pb2.Stat.P99: 110.0}})
...
def test_generate_report_fail_accuracy():
  ...
  # Setup results: Baseline=0.90, Experiment=0.80
  base = make_result("test", {"accuracy": {metric_pb2.Stat.MEAN: 0.90}})
  exp = make_result("test", {"accuracy": {metric_pb2.Stat.MEAN: 0.80}})
...
def test_generate_report_undetermined():
  ...
  # Setup results: Baseline=0, Experiment=1
  base = make_result("test", {"errors": {metric_pb2.Stat.MEAN: 0.0}})
  exp = make_result("test", {"errors": {metric_pb2.Stat.MEAN: 1.0}})
...
def test_missing_experiment_fails():
  ...
  base = make_result("broken_exp", {"latency": {metric_pb2.Stat.MEAN: 100.0}})
  pairs = {
    "broken_exp": {
      "baseline": base
      # Missing experiment
  ...
  report, success = ab_analyzer_lib.generate_report(pairs, {})

  # Experiment failure blocks the build
  assert success is False
...
def test_missing_baseline_warns():
  ...
  exp = make_result("broken_base", {"latency": {metric_pb2.Stat.MEAN: 100.0}})
  pairs = {
    "broken_base": {
      # Missing baseline
      "experiment": exp
  ...
  # Baseline failure does NOT block the build
  assert success is True

...
def test_compare_commits_multiple_pages(mocker):
    ...
    mock_run_command = mocker.patch("culprit_finder.github.run_command")

    repo = "owner/repo"
    base_sha = "base"
    head_sha = "head"

    page1_commits = [{"sha": f"sha{i}", "commit": {"message": f"msg{i}"}} for i in range(3)]
    ...
    commits = github.compare_commits(repo, base_sha, head_sha)
